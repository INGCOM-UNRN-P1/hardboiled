...
def test_new_project_files(tmp_path: Path) -> None:
    ...
    assert "mi_proyecto" in (project / "main.c").read_text(encoding="utf-8")
    assert "TARGET  := mi_proyecto.elf" in (project / "Makefile").read_text(encoding="utf-8")
    flags = (project / "compile_flags.txt").read_text(encoding="utf-8")
...
def test_new_from_example_and_editor_only(tmp_path: Path) -> None:
    ...
    assert "on_tick" in (tmp_path / "main.c").read_text(encoding="utf-8")
    ...
    assert "--target=riscv32" in (tmp_path / "compile_flags.txt").read_text(encoding="utf-8")
